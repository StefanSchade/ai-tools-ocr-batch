...
def process_images(input_dir, output_file, language, save_preprocessed, threshold, tesseract_cmd, tessdata_dir):
    ...
    with open(output_file, 'w', encoding='utf-8') as file_out:
        for filename in sorted([f for f in os.listdir(input_dir) if f.endswith('.jpeg') or f.endswith('.jpg')]):
            page_number += 1  # Increment page number for each new image
            full_path = os.path.join(input_dir, filename)

            # Write the page tag with filename before processing the image
            page_tag = f"// {json.dumps({'new_page': True, 'page_number': page_number, 'page_file': filename})}\n"
            file_out.write(page_tag)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        logging.error("Usage: python batch_ocr.py <input_directory> <output_file> [language_code] [--save-preprocessed] [threshold] [--tesseract-path <path_to_tesseract>] [--tessdata-path <path_to_tessdata>]")
        sys.exit(1)
    ...
    process_images(input_dir, output_file, language, save_preprocessed, threshold, tesseract_cmd, tessdata_dir)
