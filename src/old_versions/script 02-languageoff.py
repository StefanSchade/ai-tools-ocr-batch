...
def process_images(input_dir, output_file, save_preprocessed, threshold):
    """Process all images in the directory and output OCR results to a file."""
    # Hardcode the path to the Tesseract executable and tessdata directory
    ...
    with open(output_file, 'w', encoding='utf-8') as file_out:
        for filename in sorted([f for f in os.listdir(input_dir) if f.endswith('.jpeg') or f.endswith('.jpg')]):
            ...
            try:
                ...
                text = tesseract_ocr(img, tessdata_dir_config)
                file_out.write(text + '\n')
